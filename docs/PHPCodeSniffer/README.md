...
Learn more about the tool itself in the [official PHP_CodeSniffer wiki](https://github.com/PHPCSStandards/PHP_CodeSniffer/wiki).
...
- The `--standard=vendor/kununu/code-tools/dist/phpcs.xml.dist` flag points at the published
  template, which is the config meant for consuming projects. Do not point `--standard` at
  `vendor/kununu/code-tools/phpcs.xml`: that is this package's own config, and its
  `installed_paths` is relative to *this* repository, so the `Kununu` standard will not be found.
- Applied rules are listed below in the [Kununu Custom Rules](#kununu-custom-rules) section.

### Analyze and detect violations
```console
vendor/bin/phpcs --standard=vendor/kununu/code-tools/dist/phpcs.xml.dist src/ tests/
```

### Automatically fix violations
```console
vendor/bin/phpcbf --standard=vendor/kununu/code-tools/dist/phpcs.xml.dist src/ tests/
```
...
{
    "scripts": {
        "cs-check": "phpcs --standard=vendor/kununu/code-tools/dist/phpcs.xml.dist src/ tests/",
        "cs-fix": "phpcbf --standard=vendor/kununu/code-tools/dist/phpcs.xml.dist src/ tests/"
    }
}
```

## Customized usage
- You can customize the `../../phpcs.xml.dist` file to include/exclude directories, files, or rules.
- You can create your own ruleset.xml file and use it with the `--standard` flag.
- The easiest way to customize the rules is to copy the `../../phpcs.xml.dist` file to your project and modify it, for this we provide the following command:

```console
vendor/bin/code-tools publish:config code-sniffer
```

- The `../../phpcs.xml.dist` file will be copied to your project, and you can modify it to suit your needs.

<details>
  <summary>See some customization examples</summary>

Example of a customized `../../phpcs.xml.dist` to include only the `Kununu.PHP.NoNewLineBeforeDeclareStrict` rule:
```xml
<?xml version="1.0"?>
<ruleset name="Custom">
...
Example of a customized `../../phpcs.xml.dist` to include all Kununu rules except the `Kununu.Files.LineLength` rule:
```xml
<?xml version="1.0"?>
...
Example of a customized `../../phpcs.xml.dist` to include all Kununu rules + rules from other standards:
```xml
<?xml version="1.0"?>
<ruleset name="code-tools">
    <config name="installed_paths" value="../../kununu/code-tools/src/PHPCodeSniffer"/>
    ...
    cs:
      ...
      steps:
        ...
        - name: Run code style sniffers
          run: vendor/bin/phpcs --standard=vendor/kununu/code-tools/dist/phpcs.xml.dist #adjust the path to your phpcs.xml file if needed
 ```
</details>
...
- **phpcs**
- ![kununu/code-tools](/docs/PHPCodeSniffer/screenshots/phpcs-line-length.png)
- **git diff**
- ![kununu/code-tools](/docs/PHPCodeSniffer/screenshots/git-diff-line-length.png)
</details>
...
- **phpcs**
- ![kununu/code-tools](/docs/PHPCodeSniffer/screenshots/phpcs-MethodSignatureArguments.png)
- **phpcbf**
- ![kununu/code-tools](/docs/PHPCodeSniffer/screenshots/phpcbf-MethodSignatureArguments.png)
- **git diff**
- ![kununu/code-tools](/docs/PHPCodeSniffer/screenshots/git-diff-MethodSignatureArguments.png)
</details>
...
- **phpcs**
- ![kununu/code-tools](/docs/PHPCodeSniffer/screenshots/cs-check-no-empty-line.png)
- **phpcbf**
- ![kununu/code-tools](/docs/PHPCodeSniffer/screenshots/cs-fix-no-empty-line.png)
- **git diff**
- ![kununu/code-tools](/docs/PHPCodeSniffer/screenshots/git-diff-no-empty-line.png)
</details>
...
- **phpcs**
- ![kununu/code-tools](/docs/PHPCodeSniffer/screenshots/cs-check-empty-line-after-class-elements.png)
- **phpcbf**
- ![kununu/code-tools](/docs/PHPCodeSniffer/screenshots/cs-fix-empty-line-after-class-elements.png)
- **git diff**
- ![kununu/code-tools](/docs/PHPCodeSniffer/screenshots/git-diff-empty-line-after-class-elements.png)
</details>
